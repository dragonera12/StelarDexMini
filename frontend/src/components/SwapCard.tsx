    ...
    <div className="w-full max-w-md mx-auto card-glow glass rounded-3xl p-5 relative overflow-hidden">
        ...
        <div className="mb-4 p-3 rounded-2xl bg-zinc-900/60 border border-zinc-700/50">
          ...
          <div className="flex justify-between items-center">
            <div className="text-center">
              ...
              <p className="text-[10px] text-zinc-500 font-mono">XLM (Classic)</p>
            </div>
            <div className="w-px h-8 bg-zinc-700" />
            <div className="text-center">
              <p className="text-lg font-bold text-[#7DF9FF]">{balanceIn}</p>
              <p className="text-[10px] text-zinc-500 font-mono">{tokenIn.symbol} (Wallet)</p>
            </div>
            <div className="w-px h-8 bg-zinc-700" />
            <div className="text-center">
              <p className="text-lg font-bold text-[#f59e0b]">{balanceOut}</p>
              <p className="text-[10px] text-zinc-500 font-mono">{tokenOut.symbol} (Wallet)</p>
            </div>
          </div>
          <p className="text-[10px] text-zinc-600 mt-2 leading-tight">
            ⚡ Swaps are performed via Soroban SAC contracts. Your actual Freighter wallet balances will update automatically after each transaction.
          </p>
        </div>
      )}
      ...
      {quote && (
        <div className="mt-5 space-y-2 px-1 py-3 rounded-2xl bg-zinc-900/40 border border-zinc-800">
          {[
            ["Rate", parseFloat(quote.executionPrice) > 0 
            ...
            <div key={String(label)} className="flex justify-between text-sm px-2">
              ...
              <span className={cn(
                "font-mono", 
                isWarning || (label === "Rate" && parseFloat(quote.executionPrice) === 0) ? "text-amber-400" : "text-zinc-200"
              )}>
                {String(value)}
              </span>
            </div>
          ...
          {parseFloat(quote.executionPrice) === 0 && (
            <div className="mt-3 mx-2 p-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[10px] text-amber-500 text-center leading-tight font-mono uppercase tracking-tight">
              ⚠️ Pool is empty! Go to the <strong>Pool</strong> tab and add liquidity to set the conversion rate.
            </div>
          )}
        </div>
      )}
          ...
          {txStatus.status === "success" && (
            <>
              ...
              <p className="text-zinc-400 text-sm mb-6">
                Your Freighter wallet balances have been updated. The new {tokenOut.symbol} balance should now reflect in your wallet assets.
              </p>

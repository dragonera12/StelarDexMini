...
export const LiquidityCard: React.FC = () => {
  ...
  const { poolStats, myPosition, txStatus, addLiquidity, removeLiquidity, clearStatus } = usePool();
  ...
  return (
    <div className="w-full max-w-md mx-auto card-glow glass rounded-3xl p-5 relative overflow-hidden">
      ...
      {txStatus.status === "fail" && (
        <div className="mt-4 p-3 bg-danger/10 border border-danger/20 rounded-xl text-danger text-xs text-center">
          <p className="font-bold mb-1 uppercase tracking-tight">Transaction Failed</p>
          <p>{txStatus.error?.message || "Unknown error"}</p>
          <button 
            onClick={clearStatus}
            className="mt-2 text-zinc-400 hover:text-white underline"
          >
            Try Again
          </button>
        </div>
